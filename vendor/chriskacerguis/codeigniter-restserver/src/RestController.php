...
class RestController extends \CI_Controller
{
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        switch ($uri_string) {
			...
			case 'api/auth/token':
			case 'api/kakaosync/info':
			case 'api/kakaosync/state':
            case 'api/auth/authToken':
            case 'api/geditor/setGeditor':
				$force_https = false;
				break;
		}

        if ($force_https && $this->request->ssl === false) {
            $this->response([
                $this->config->item('rest_status_field_name')  => false,

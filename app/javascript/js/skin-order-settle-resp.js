	...
	*/
	...
	$(window).load(function(){
		...
		if (typeof is_order_present === 'undefined') {
			is_order_present = false;
		} 

		// 모바일 기본 주소 노출
		if ( is_members == true && is_order_present === false) {
			$.ajax({
		...
		if (is_order_present === true) {
			$("#payment_type > li").hide();
			$("#payment_type > li").each(function () {
				if ($(this).find('div').hasClass('card')) {
					$(this).show();
				}
			});
		}
		$("#payment_type > li:visible").eq(0).find("input[name='payment']").trigger('click');
		...
		if(is_goods == false) { goods_delivery_info_resp(); }
		if(typeof is_address != 'undefined' && is_address  && is_order_present === false){
			address_modify('delivery');

...
class accountall extends admin_base {
	...
	// 비동기 api 호출
	public function async_accountgroup()
	{
		...
		$this->validation->set_rules('s_year', 's_year','trim|required|numeric|xss_clean');
		$this->validation->set_rules('s_month', 's_month','trim|required|numeric|xss_clean');
		$this->validation->set_rules('pay_period', 'pay_period', 'trim|xss_clean');
		$this->validation->set_rules('provider_seq_list', 'provider_seq_list', 'trim|xss_clean');
		...
		$requestGet['s_year'] = $requestGet['s_year'] ?? date('Y');
		$requestGet['s_month'] = $requestGet['s_month'] ?? date('m');
		// 빈 string 넘어오는 경우가 있다
		$requestGet['pay_period'] = (strlen($requestGet['pay_period']) === 0)? 'all' : $requestGet['pay_period'];

		$providerSeqList = explode(',', $requestGet['provider_seq_list']);
		// 숫자로 변환 합니다 
		$requestGet['provider_seq_list'] = array_map(function($number) {
			return (int) $number;
		}, $providerSeqList);

		$requestGet['acc_table'] = trim($requestGet['s_year'] . $requestGet['s_month']);

		/*
		* 신규 정산을 이용할 경우 마이그레이션 다음 달부터 적용되도록 처리 :: 2017-06-14 lkh
		* 마이그레이션 날짜 가져오기 시작
		*/
		$checkDate = trim($requestGet['s_year'] . '-' . $requestGet['s_month']);
		$_accountSettings = getAccountSetting();
		$accountallPatchV1 = $_accountSettings['accountall_patch_v1'];		// account patch v1 : 신정산 수정 v1 패치일자 20190430 개선 pjm
		$accountAllViewV1 = $_accountSettings['accountall_view_v1'];		// account view v1 : 신정사 수정 v1 view 기준일자 20190430 개선 pjm
		$migrationYear = $_accountSettings['migrationYear'];
		...
		* 정산주기 설정 시작
		*/
		$accountAllDate = getAccountAllDate($requestGet);
		/*
		* 정산주기 설정 끝
		*/

		// 전체 조회	: 엑셀과 합계를 구하기 위해서는 전체를 조회해야함
		$requestGet['total_view'] = '1';

		/*
		* 정산 실행 내역 시작
		*/
		$accountConfirm = [];
		$accountallConfirmSetting = [];
		...
		//정산 마감전
		if (!$accountConfirm) {
			$settingArr['year'] = $requestGet['s_year'];
			$settingArr['month'] = $requestGet['s_month'];
			$accountallConfirmSetting = $this->accountallmodel->get_account_setting('month', $settingArr);
			$yearmonth = $settingArr['year'] . '-' . $settingArr['month'];
			$accountallConfirmSetting['confirm_date'] = date('Y-m', strtotime($yearmonth . '+1 month')) . '-' . sprintf('%02d', $accountallConfirmSetting['accountall_confirm']);
			$accountallConfirmSetting['confirm_name'] = getConfirmDay($accountallConfirmSetting['accountall_confirm']);
		...
		$requestGet['listview'] = $listview;
		$loop = $tot = $carryoverloop = $carryovertot = $overdrawloop = $overdrawtot = $accountAllCount = [];

		if (($accountAllMigrationDate == '0000-00-00' || ($checkDate >= $migrationCheckDate && $requestGet['s_year'] >= $migrationYear)) && !$requestGet['old']) {
			$query_result = $this->accountallmodel->get_account_all_catalog_query($requestGet);
		}
		...
		$extend = [
			...
			'mode_accountalllist' => 'sum',
			'accountAllDate' => $accountAllDate,
			'pay_period' => $requestGet['pay_period'],
			'provider' => $provider,
			'accountAllCount' => $accountAllCount,
		];

		foreach ($query_result as $k => $query_list) {
			while ($acinsdata = mysqli_fetch_assoc($query_list)) {
				$acinsdata_order_seq_ar[$acinsdata['ac_type']][$acinsdata['status']][] = $acinsdata['order_seq'];
				switch ($acinsdata['status']) {//전월/당월/차월 구분
					case 'carryover'://통합정산데이타의 전월
					case 'not-carryover':
				...
				if ($requestGet['account_hidden_sales']) {
					unset($overdrawloop,$overdrawtot);
				}
				if ($requestGet['account_hidden_cal']) {
					unset($carryoverloop,$carryovertot);
				}
			}
		}
		...
		*/
		foreach ($provider as $tmp_provider) {
			// 정산주기 조건절
			if ($requestGet['pay_period'] && $requestGet['pay_period'] != 'all') {
				if ($requestGet['pay_period'] != $tmp_provider['calcu_count']) {
					continue;
				}
			}
		...
		$output = [
			'result' => 'success',
			'accountGroups' => $loop_accountgroup,
			// 정산
			'calculate' => [
				// 실행마감일
				'accountConfirm' => $accountConfirm,
				// 마감예정일
				'accountallConfirmSetting' => $accountallConfirmSetting,
			]
		];
	...
	**/
	public function accountgroup()
	{
		...
		$this->template->assign([
			...
			'tot' => $tot,
			'sc' => $requestGet,
			'scObj' => json_encode($requestGet),
			// 정산실행마감일
			'accountConfirm' => $accountConfirm,
			// 정산마감예정일
			'accountallConfirmSetting' => $accountallConfirmSetting,
			'providerList' => addslashes(json_encode($provider)),
			'provider' => $provider,
			'year' => $year, 
			'month' => $month,
		]);
		...
		requirejs([
			['/app/javascript/js/ieBrowserNotification.js', 1],
			['/app/javascript/js/admin/gSearchForm.js', 10],
			['/admin/skin/default/css/accountall.css', 10],
			['/app/javascript/plugin/simpleProgress.js', 10],
		]);
	...
	**/
	...
	function accountallviewerall_checker_tool_ajax(){
		...
		$accountData = unserialize(base64_decode($accountData), ['allowed_classes' => false]);

...
function adjustPath($source, $tpl, $indicator='', $type='absolute')
{
	...
	$default_indicator = 'css,js,gif,jpg,jpeg,png,swf,js,svg';
	$path_filter = array();
...
// convert to absolute path
	...
	for ($i=1,$s=count($split); $i<$s; $i+=2) {
		...
		// custom template 분기처리
		if( isInstalledRunkit() ){
			$new_path = '/'.implode('/',array_slice(explode('/', $src),$base_cnt));
			if(is_file($base_path.$new_path)) $split[$i]='/'.implode('/',array_slice(explode('/', $src),$base_cnt));
			else {
				$split[$i]='/'.implode('/',array_slice(explode('/', $src),$base_cnt));
				$split[$i]=str_replace("/custom/".__CUSTOM_CONFIG_SITEID__,"",$split[$i]);
			}
		}
